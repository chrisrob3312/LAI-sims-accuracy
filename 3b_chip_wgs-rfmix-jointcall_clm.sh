...
# ---- CHIP-DENSITY EXTENSION -------------------------------------------------
# CHIP_NAME picks the chip label used in the output subdir. CHIP_SNPS_TPL is
# a printf-style template pointing to a per-chr SNP list of "chr:pos" entries,
# one per line. Passing CHIP_NAME "wgs" (or unsetting CHIP_SNPS_TPL) makes this
# script behave exactly like the parent 3b (full-WGS density).
#
# Only the admixed haps get filtered; reference haps stay full-WGS. When
# shapeit2rfmix intersects admixed + ref sites, the intersection reduces to
# ~the chip sites present in the phased panel — mirroring how a real chip-
# genotyped Latino cohort would look post-imputation when queried against a
# full-WGS reference (Illumina GSA v3 ~600K, Omni2.5 ~2.4M, MEGA ~1.7M).
CHIP_NAME="${CHIP_NAME:-wgs}"
CHIP_SNPS_TPL="${CHIP_SNPS_TPL:-}"
# Put outputs in a chip-scoped subdir so they never overwrite the parent
# 3b run's Lat3/done markers.
RFMIX_OUTDIR="${RFMIX_OUTDIR%/}/chip_${CHIP_NAME}"
...
run_panel_track () {
    ...
    # Chip-density hook: filter admixed haps to chip SNPs when CHIP_SNPS_TPL set.
    # SHAPEIT .haps col 3 = physical position, so a chr:pos SNP list matches
    # after prepending "${CHR}:". Reference haps stay full-WGS; shapeit2rfmix's
    # subsequent intersection reduces to chip ∩ WGS ≈ chip density in the panel.
    if [[ -n "$CHIP_SNPS_TPL" ]]; then
        # shellcheck disable=SC2059
        local chip_snps
        chip_snps=$(printf "$CHIP_SNPS_TPL" "$CHR")
        [[ -s "$chip_snps" ]] || { echo "ERROR: missing chip SNP list $chip_snps"; return 1; }
        local admixed_haps_chip="${WORKDIR}/${track}.${ADMIX_POP}.chr${CHR}.chip_${CHIP_NAME}.haps"
        if [[ ! -s "$admixed_haps_chip" ]]; then
            echo "[$(date +%T)] [chr${CHR}] [$track/$panel] filtering admixed haps to $(basename "$chip_snps")"
            awk -v chr="$CHR" 'NR==FNR{k[$0]=1; next} (chr":"$3) in k' \
                "$chip_snps" "$admixed_haps" > "$admixed_haps_chip"
            local n_full=$(wc -l < "$admixed_haps")
            local n_chip=$(wc -l < "$admixed_haps_chip")
            echo "[$(date +%T)] [chr${CHR}] [$track/$panel] admixed haps: $n_full sites -> $n_chip chip sites"
            if [[ "$n_chip" -lt 100 ]]; then
                echo "ERROR: only $n_chip chip sites survived filter -- check chr format in $chip_snps"
                return 1
            fi
        fi
        admixed_haps="$admixed_haps_chip"
    fi
