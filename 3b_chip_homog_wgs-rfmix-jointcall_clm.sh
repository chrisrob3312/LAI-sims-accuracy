...
# NEW: homogeneity-filtered HGDPMXB (Q_sup_AMR >= 0.95 AND sup/unsup agree)
NAT_HOMOG_RFMIX="${NAT_HOMOG_RFMIX:-${REFS}/amr_homog_rfmix.txt}"
...
# in that case).
# Homog-panel-only fork: this script runs ONLY NAT_HOMOG so it can execute in
# parallel with the main 3b job without duplicating combos. Override via
# PANELS_TO_RUN if a different set is wanted.
DEFAULT_PANELS="NAT_HOMOG"
...
# ---- CHIP-DENSITY EXTENSION (mirrors 3b_chip) -------------------------------
# CHIP_NAME picks the chip label (becomes output subdir name). CHIP_SNPS_TPL
# is a printf-template per-chr SNP list of chr:pos entries. Unsetting
# CHIP_SNPS_TPL makes this script run at full-WGS density.
# Only admixed haps get filtered; reference stays full-WGS. shapeit2rfmix's
# intersection then reduces to chip sites present in the panel.
...
(
    ...
    [[ -s "$NAT_HOMOG_RFMIX" ]] && extract_haps NAT_HOMOG "$NAT_HOMOG_RFMIX" || true
    ...
    [[ -s "${WORKDIR}/NAT_HOMOG_chr${CHR}.haps" ]] && build_ref NAT_HOMOG NAT_HOMOG || true
...
run_panel_track () {
    ...
    # Chip-density hook (mirrors 3b_chip)
    if [[ -n "$CHIP_SNPS_TPL" ]]; then
        ...
        if [[ ! -s "$admixed_haps_chip" ]]; then
            ...
            [[ "$n_chip" -ge 100 ]] || { echo "ERROR: only $n_chip chip sites"; return 1; }
        fi
